    ...
    return new Promise((resolve) => {
        try {
            ...
            const requestWithRedirect = (currentUrl, depth = 0) => {
                ...
                const req = protocol.request(currentUrl, requestOptions, (res) => {
                ...
                req.on('error', (err) => {
                    console.log(`[4KHDHub] Seek validation error for ${currentUrl}: ${err.message}`);
                    req.destroy(); // Ensure request is destroyed on error
                    resolve({ isValid: false, filename: null, statusCode: null, contentLength: null });
                });

                req.on('timeout', () => {
                    console.log(`[4KHDHub] Seek validation timeout for ${currentUrl}`);
                    req.destroy();
                    resolve({ isValid: false, filename: null, statusCode: null, contentLength: null });
                });
