...
export async function extractTvShowDownloadLinks(showPageUrl, season, episode) {
  try {
    ...
    if (downloadLinks.length === 0) {
      ...
      // If absolutely no links found standard or regex fallback, try AI
      if (downloadLinks.length === 0) {
        console.log(`[UHDMovies] All regex methods failed. Attempting AI extraction fallback.`);
        await extractEpisodeLinksAI($, season, episode, downloadLinks);
      }
    }
