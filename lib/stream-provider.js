...
import { isJunkRelease } from './util/ai-matcher.js';
...
async function getSeriesStreams(config, type, id) {
  ...
  // Apply AI Junk Filter
  if (process.env.AI_MATCHING_ENABLED === 'true') {
    flatStreams = await filterJunkByAI(flatStreams);
  }
...
async function filterJunkByAI(streams) {
  if (!streams || streams.length === 0) return streams;
  console.log('[AI-FILTER] Checking ' + streams.length + ' streams for junk...');
  const results = [];
  const chunks = [];
  const CHUNK_SIZE = 5;
  for (let i = 0; i < streams.length; i += CHUNK_SIZE) {
    chunks.push(streams.slice(i, i + CHUNK_SIZE));
  }
  for (const chunk of chunks) {
    const promises = chunk.map(async (stream) => {
      const title = stream.title || stream.name || '';
      const isJunk = await isJunkRelease(title);
      if (!isJunk) return stream;
      return null;
    });
    const chunkResults = await Promise.all(promises);
    results.push(...chunkResults.filter(Boolean));
  }
  const removed = streams.length - results.length;
  if (removed > 0) console.log('[AI-FILTER] Removed ' + removed + ' junk streams');
  return results;
}
