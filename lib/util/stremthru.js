...
export async function getTorrentsByStremId(type, id, debridService, debridToken, cachedOnly = true) {
  if (!config.STREMTHRU_ENABLED || !config.STREMTHRU_URL) {
    console.log(`[${LOG_PREFIX}] getTorrentsByStremId: StremThru not enabled or URL not set`);
    return [];
  }
  ...
  try {
    ...
    const baseUrl = config.STREMTHRU_URL.replace(/\/$/, '');
    const torzUrl = `${baseUrl}/stremio/torz/${token}/stream/${type}/${id}.json`;
    console.log(`[${LOG_PREFIX}] Calling Torz API: ${type}/${id} (service: ${serviceCode})`);

    const response = await axios.get(torzUrl, {
      headers: {
        'User-Agent': 'Sootio/1.0',
    ...
    console.log(`[${LOG_PREFIX}] Torz API response: ${streams.length} streams for ${type}/${id}`);
    ...
    console.log(`[${LOG_PREFIX}] Retrieved ${torrents.length}/${streams.length} torrents with valid InfoHash from Torz API for ${type}/${id}`);
    return torrents;
  } catch (error) {
    const status = error?.response?.status;
