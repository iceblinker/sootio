...
export async function matchesSeriesTitle(candidate, canonicalTitle) {
  ...
  if (process.env.AI_MATCHING_ENABLED === 'true') {
    try {
      const aiMatcher = await import('./ai-matcher.js');
      // Check primary name against canonical title
      if (typeof candidate === 'string') {
        if (await aiMatcher.areTitlesSemanticallyEquivalent(candidate, canonicalTitle)) return true;
      } else if (candidate.name) {
        if (await aiMatcher.areTitlesSemanticallyEquivalent(candidate.name, canonicalTitle)) return true;
      }
    } catch (e) {
      // Ignore import errors or runtime errors in matching, fallback to false
    }
  }
