...
async function searchPersonalFiles(apiKey, searchKey, threshold = 0.3) {
    ...
    let torrents = files.map(file => toTorrent(file, { isPersonal: true }))
...
function toTorrent(item, { isPersonal = false } = {}) {
    return {
        source: 'premiumize',
        id: item.id,
        name: item.name,
        ...
        ...(isPersonal ? { isPersonal: true, isCached: true, tracker: 'Personal' } : {}),
    }
}
